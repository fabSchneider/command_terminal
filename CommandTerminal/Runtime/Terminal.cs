...
namespace CommandTerminal
{
    ...
    public class Terminal : MonoBehaviour
    {
        ...
        [SerializeField] InputAction ToggleHotkey;     
        [SerializeField] InputAction ToggleFullHotkey;
        ...
        void OnEnable() {
			...
			ToggleHotkey?.Enable();
			ToggleFullHotkey?.Enable();
        }

        void OnDisable() {
            Application.logMessageReceivedThreaded -= HandleUnityLog;

			ToggleHotkey?.Disable();
			ToggleFullHotkey?.Disable();
		}
        ...
        void OnGUI() {
            if (ToggleHotkey.WasReleasedThisFrame()) {
                SetState(TerminalState.OpenSmall);
                initial_open = true;
            } else if (ToggleFullHotkey.WasPressedThisFrame()) {
        ...
        void DrawConsole(int Window2D) {
            ...
            } else if (ToggleHotkey.WasPressedThisFrame()) {
                ToggleState(TerminalState.OpenSmall);
            } else if (Event.current.Equals(ToggleFullHotkey.WasPressedThisFrame())) {
